        ...
        private void paidToBtn_Click(object sender, EventArgs e)
        {
            ...
            if (paidToCreated == false)
            {
                ...
                for (int j = 0; j < numPaidTo; j++)
                {
                    ...
                    cmd.Parameters.AddWithValue("@PaidTo", paidTo[j]);
                    ...
                    paidToCost[j] = String.Format("{0}", reader["TotalPayment"]);
                    connection.Close();

                    paidToChart.Series[paidToSeries].Points.AddXY(paidTo[j], paidToCost[j]);
                    paidToChart.Series[paidToSeries].Points[j].Label = paidTo[j];
                    paidToCost[j] = String.Format("{0:#.00}", Convert.ToDecimal(paidToCost[j]));
                    paidToCost[j] = "$" + paidToCost[j];
                    paidToChart.Series[paidToSeries].Points[j].LegendText = paidToCost[j];
                    paidToChart.Series[paidToSeries].Points[j].Font = new Font("Arial", 14, FontStyle.Bold);
                    paidToChart.Series[paidToSeries].LabelForeColor = Color.White;
                    paidToChart.Series[paidToSeries].Points[j].ToolTip = paidToCost[j];
